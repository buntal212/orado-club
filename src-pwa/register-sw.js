...
function announceUpdate(registration) {
  ...
  window.dispatchEvent(new CustomEvent('orado-club:pwa-update-available', { detail: { registration } }))
}
...
register(import.meta.env.QUASAR_SERVICE_WORKER_FILE, {
  ...
  error(error) {
    console.error('Service worker ORADO Club gagal didaftarkan:', error)
  },
})
