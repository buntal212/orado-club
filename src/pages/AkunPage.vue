  ...
  <q-page class="account-page">
    <main class="account-content">
      ...
      <section class="identity-card">
        <img class="identity-watermark" :src="logoOrado" alt="" aria-hidden="true" />
        ...
        <div class="club-code">
          <span>KODE CLUB</span><strong>{{ club?.kode_club || '-' }}</strong>
        </div>

        <div class="identity-footer">
          <div>
            <span>PEMILIK AKUN</span><strong>{{ user?.username || user?.email || '-' }}</strong>
          </div>
      ...
      <section class="profile-card">
        <div class="section-title">
          <q-icon name="business" color="positive" /><strong>Informasi Club</strong>
        </div>
        <q-list separator>
          <q-item>
            ...
            <q-item-section
              ...
              ><q-item-label>{{ club?.alamat || '-' }}</q-item-label></q-item-section
            >
          </q-item>
          <q-item>
            <q-item-section avatar><q-icon name="map" color="positive" /></q-item-section>
            <q-item-section
              ><q-item-label caption>Kelurahan / Kecamatan</q-item-label
              ><q-item-label>{{ wilayah }}</q-item-label></q-item-section
            >
          </q-item>
          <q-item v-if="club?.catatan">
            ...
            <q-item-section
              ...
              ><q-item-label>{{ club.catatan }}</q-item-label></q-item-section
            >
          </q-item>
        </q-list>
      </section>
      ...
      <section class="member-section">
        <div class="member-section-heading">
          <div>
            <span>IDENTITAS ANGGOTA</span>
            <h2>Kartu Anggota Club</h2>
          </div>
          <q-badge rounded color="positive" :label="`${verifiedMembers.length} anggota`" />
        </div>

        <div v-if="anggotaStore.loading" class="member-state">
          <q-spinner color="positive" size="30px" /> Memuat kartu anggota...
        </div>
        <div v-else-if="!verifiedMembers.length" class="member-state">
          <q-icon name="badge" size="32px" /> Belum ada anggota terverifikasi.
        </div>
        <article v-for="member in verifiedMembers" v-else :key="member.id" class="member-card">
          <img class="member-watermark" :src="logoOrado" alt="" aria-hidden="true" />
          <div class="member-card-top">
            <div class="member-avatar"><img src="@/assets/orado/logo-white.svg" alt="ORADO" /></div>
            <div class="member-brand">
              <strong>ORADO KOTA PROBOLINGGO</strong><span>Kartu Anggota</span>
            </div>
            <div class="member-tools">
              <q-btn
                ...
                dense
                icon="download"
                aria-label="Download kartu anggota"
                @click="downloadMemberCard(member)"
              />
              <img
                v-if="memberQrCodes[member.id]"
                class="member-qr"
                :src="memberQrCodes[member.id]"
                alt="QR identitas anggota"
              />
            </div>
          </div>
          <div class="member-name">{{ member.name }}</div>
          <div class="member-nik">
            <span>NIK</span><strong>{{ member.nik }}</strong>
          </div>
          <div class="member-details">
            <div>
              <span>NAMA CLUB</span><strong>{{ clubDisplayName }}</strong>
            </div>
            <div>
              <span>NO. HP</span><strong>{{ member.no_hp || '-' }}</strong>
            </div>
            <div>
              <span>JENIS KELAMIN</span><strong>{{ genderLabel(member.jenis_kelamin) }}</strong>
            </div>
          </div>
...
const verifiedMembers = computed(() => anggotaStore.anggota.filter((member) => member.flag === 2))
...
const wilayah = computed(
  () => [club.value?.kelurahan, club.value?.kecamatan].filter(Boolean).join(', ') || '-',
)
const statusColor = computed(() =>
  String(club.value?.status).toLowerCase().includes('verifikasi') &&
  !String(club.value?.status).toLowerCase().includes('menunggu')
    ? 'positive'
    : 'orange-8',
)
...
async function downloadMemberCard(member) {
  ...
  try {
    const watermark = await loadImage(logoOrado)
    ctx.save()
    ctx.globalAlpha = 0.035
    ctx.filter =
      'brightness(0) saturate(100%) invert(25%) sepia(40%) saturate(900%) hue-rotate(115deg)'
    ctx.drawImage(watermark, 500, 210, 300, 300)
    ctx.restore()
  } catch {
    // Watermark tidak mengganggu proses cetak jika aset logo gagal dimuat.
  }
  ...
  const filename = String(member.name || 'anggota')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
...
async function generateMemberQrCodes() {
  const entries = await Promise.all(
    verifiedMembers.value.map(async (member) => [
      member.id,
...
.account-page {
  ...
  background: linear-gradient(180deg, #006b4f 0, #006b4f 185px, #f1f6f3 185px);
}
.account-content {
  max-width: 500px;
  margin: auto;
...
.identity-card {
  ...
  border-radius: 20px;
  color: #fff;
  background: linear-gradient(145deg, #014d39, #00825e);
  box-shadow: 0 16px 34px rgba(3, 73, 53, 0.28);
}
.identity-watermark {
  position: absolute;
  z-index: 0;
...
.profile-card {
  ...
  border-radius: 17px;
  background: #fff;
  box-shadow: 0 7px 20px rgba(3, 73, 53, 0.08);
}
.section-title {
  display: flex;
  align-items: center;
...
.profile-card :deep(.q-item) {
  padding-right: 2px;
  padding-left: 2px;
}
.profile-card :deep(.q-item__section--avatar) {
  min-width: 36px;
}
.profile-card :deep(.q-item__label) {
  color: #294c42;
  font-size: 12px;
}
.profile-card :deep(.q-item__label--caption) {
  color: #80948d;
  font-size: 10px;
}
.member-section {
...
.member-section-heading :deep(.q-badge) {
  padding: 5px 8px;
  font-size: 9px;
}
.member-state {
...
.member-card {
  ...
  border-top: 4px solid #007157;
  border-radius: 18px;
  color: #174c3d;
  background: linear-gradient(145deg, #fff 0%, #f2fbf7 72%, #e4f5ed 100%);
  box-shadow: 0 10px 26px rgba(3, 73, 53, 0.12);
}
.member-card::before {
  position: absolute;
  width: 170px;
  ...
  right: -80px;
  content: '';
  border: 22px solid rgba(0, 107, 79, 0.045);
  border-radius: 50%;
}
.member-card::after {
  position: absolute;
  ...
  bottom: -80px;
  content: '';
  border: 18px solid rgba(0, 107, 79, 0.06);
  border-radius: 50%;
}
.member-watermark {
  position: absolute;
  ...
  width: 155px;
  opacity: 0.06;
  filter: brightness(0) saturate(100%) invert(25%) sepia(40%) saturate(900%) hue-rotate(115deg);
  pointer-events: none;
}
.member-card-top {
  position: relative;
...
.member-avatar {
  ...
  border-radius: 50%;
  color: #fff;
  background: linear-gradient(145deg, #006b4f, #00a174);
  box-shadow: 0 5px 12px rgba(0, 107, 79, 0.2);
}
.member-avatar .q-icon {
  font-size: 24px;
}
.member-brand strong,
.member-brand span {
...
.member-card-footer :deep(.q-badge) {
  padding: 5px 8px;
  font-size: 8px;
